...
const Template2 = ({ data }) => {
  ...
  return (
    ...
    <div className="container my-4" >
      <style>{`
        .purple-header th {
          background: linear-gradient(to right, #6d28d9, #a78bfa) !important;
      ...
      <div className="border rounded p-4" style={{ backgroundColor: '#f3e8ff' }}>
        {/* Invoice Title */}
        <h2 className="fw-bold mb-3" style={{ marginLeft: '-6%' }}>
          Tax Invoice
        </h2>
    ...
    <div className="border border-dark p-3" style={{ flex: '1 1 30%', minWidth: '280px' }}>
      ...
      <p className="mb-1 text-start"><strong>Payment Due Date:</strong> {data.paymentDueDate || "N/A"}</p>
            ...
            <table className="table table-bordered table-striped mb-0">
              ...
              <tbody>
                ...
                <tr className="purple-footer">
                  <td>Total Tax</td>
                  <td>
                    {data.taxSummary?.reduce(
                      (sum, tax) => sum + Number(tax.taxableAmount || 0),
                      0
                    ).toFixed(2)}
                  </td>
                  <td>
                    {data.taxSummary?.reduce(
                      (sum, tax) => sum + Number(tax.cgstAmount || 0),
                      0
                    ).toFixed(2)}
                  </td>
                  <td>
                    {data.taxSummary?.reduce(
                      (sum, tax) => sum + Number(tax.sgstAmount || 0),
                      0
                    ).toFixed(2)}
                  </td>
                  <td>
                    {data.taxSummary?.reduce(
                      (sum, tax) => sum + Number(tax.totalTax || 0),
                      0/*  */
                    ).toFixed(2)}
                  </td>
                </tr>
        ...
        <div className="p-3 d-flex justify-content-between flex-wrap text-start">
          ...
          <div className="flex-fill pe-3 border" style={{ minWidth: '200px', borderColor: '#8b5cf6' }}>
            <div className='p-2'>
          ...
          {/* Center - Receiver Signature */}
          <div className="flex-fill text-center py-4 border" style={{ minWidth: '200px', borderColor: '#8b5cf6' }}>
            <p className="fw-bold">Receiver's Signature</p>
            <div style={{ borderBottom: '1px solid #8b5cf6', width: '60%', margin: '0 auto' }}></div>
          </div>

          {/* Right - Authorized Signatory */}
          <div className="flex-fill ps-3 d-flex flex-column align-items-end justify-content-end text-end border" style={{ minWidth: '200px', borderColor: '#8b5cf6' }}>
