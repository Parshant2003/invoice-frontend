...
const Template3 = ({ data }) => {
  ...
  return (
    <div className="container my-4" >
      ...
      <div className="border rounded p-4" style={{ backgroundColor: '#fff7ed' }}>
        ...
        <div className="d-flex justify-content-between align-items-start mb-4 flex-wrap">
          <div className="ms-5 rounded" style={{ width: '120px', height: '120px', border: '2px solid #fb923c' }}>
            <img src={data.companyLogo} alt="Company Logo" className="img-fluid" style={{ width: '100%', height: '100%', objectFit: 'fill' }} />
          </div>
    ...
    <div className="border border-dark p-3" style={{ flex: '1 1 30%', minWidth: '280px' }}>
      ...
      <p className="mb-1 text-start"><strong>Payment Due Date:</strong> {data.paymentDueDate || "N/A"}</p>
        ...
        <div className="p-3 mb-4">
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-striped mb-0">
              ...
              <tfoot>
                <tr className="orange-footer">
                  <td colSpan="11" className="text-end">Total Amount (₹):</td>
                  <td>{data.totalAmount || "0.00"}</td>
                </tr>
                <tr>
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-striped mb-0">
              ...
              <tbody>
                ...
                <tr className="orange-footer">
                  <td>Total Tax</td>
                  <td>{data.taxSummary?.reduce((sum, tax) => sum + Number(tax.taxableAmount || 0), 0).toFixed(2)}</td>
                  <td>{data.taxSummary?.reduce((sum, tax) => sum + Number(tax.cgstAmount || 0), 0).toFixed(2)}</td>
                  <td>{data.taxSummary?.reduce((sum, tax) => sum + Number(tax.sgstAmount || 0), 0).toFixed(2)}</td>
                  <td>{data.taxSummary?.reduce((sum, tax) => sum + Number(tax.totalTax || 0), 0).toFixed(2)}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="p-3 d-flex justify-content-between flex-wrap text-start">
          <div className="flex-fill pe-3 border orange-border" style={{ minWidth: '200px' }}>
            <div className='p-2'>
          ...
          <div className="flex-fill text-center py-4 border orange-border" style={{ minWidth: '200px' }}>
            <p className="fw-bold">Receiver's Signature</p>
            <div style={{ borderBottom: '1px solid #fb923c', width: '60%', margin: '0 auto' }}></div>
          </div>

          <div className="flex-fill ps-3 d-flex flex-column align-items-end justify-content-end text-end border orange-border" style={{ minWidth: '200px' }}>
